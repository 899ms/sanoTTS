/* c3_nano_main.c -- on-silicon gate for the E12-nano (294,642 params) on the
 * ESP32-C3. Sibling of the ESP32-S3 harness; same fixture, same fixed
 * sha256(row_id) seed, same 0.98 gate, same measurement protocol.
 *
 * THREE DIFFERENCES FROM THE S3 HARNESS, all forced by the part:
 *   - no PSRAM: the C3 has none, so there is no arena comparison to make and
 *     no PSRAM penalty to price. Internal SRAM is the only option.
 *   - single core: no worker task, so there is no 1-core/2-core pair either.
 *   - no vector unit: the residency counters here separate SRAM-resident
 *     operands from flash-XIP ones, NOT SIMD from scalar. Every MAC on this
 *     part is scalar. The label says so.
 *
...
extern int64_t g_mv_macs_resident, g_mv_macs_flash;
extern int64_t g_mv_calls_resident, g_mv_calls_flash;
...
static double run_row(const char *tag, const int32_t *ids, int n_ids,
    ...
    int64_t tot = g_mv_macs_resident + g_mv_macs_flash;
    printf("DEVICE: [%s] int8 matvec operand residency (ALL MACs are SCALAR on "
           "RV32IMC): SRAM %lld MACs / %lld calls (%s%%), "
           "FLASH-XIP %lld MACs / %lld calls (%s%%)\n", tag,
           (long long)g_mv_macs_resident, (long long)g_mv_calls_resident,
           f3(tot ? 100.0 * (double)g_mv_macs_resident / (double)tot : 0.0, b1),
           (long long)g_mv_macs_flash, (long long)g_mv_calls_flash,
           f3(tot ? 100.0 * (double)g_mv_macs_flash / (double)tot : 0.0, b2));
...
void app_main(void)
{
    ...
    esp_chip_info_t ci;
    esp_chip_info(&ci);
    printf("chip    : %d core(s), rev %d, %d MHz  (RV32IMC, no FPU, no vector unit)\n",
           ci.cores, ci.revision, (int)(esp_clk_cpu_freq() / 1000000));
    ...
    /* There is no PSRAM on this part, so unlike the S3 there is no fallback
     * and no arena comparison: if it does not fit internal SRAM it does not
     * run, and that is the result. */
    size_t largest = heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    size_t want = 240 * 1024;
    if (want > largest - 8192) want = largest - 8192;
    void *arena = heap_caps_aligned_alloc(16, want, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!arena) { printf("FATAL: no arena\n"); return; }
    printf("arena   : %u bytes @ %p in INTERNAL SRAM (resident=%d)\n",
           (unsigned)want, arena, snt_weights_resident(arena));
    printf("======================================================\n\n");
    ...
    double cr0 = run_row("r00/SRAM", ids, n_ids, durs, gold, n_gold,
    ...
    double cr6 = run_row("r06/SRAM", ids6, n_ids6, (const int32_t *)durs6_start,
                         (const float *)audio6_start,
                         (size_t)(audio6_end - audio6_start) / 4, ROW6_SEED,
                         ROW6_FRAMES, ROW6_SAMPLES, arena, want,
                         "INTERNAL SRAM", 1);
