...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--arena-min", type=int, default=0,
                        help="minimum arena in bytes; 0 estimates it from the frame count")
...
# --arena-min is not given.
ARENA_FIXED_BYTES = 47_616
...
def main() -> None:
    ...
    frames = sum(struct.unpack(f"<{len(durs_raw)//4}i", durs_raw)) if is_nano else 0
    arena_min = args.arena_min or (
        ARENA_FIXED_BYTES + frames * ARENA_BYTES_PER_FRAME + ARENA_MARGIN_BYTES
        if is_nano else 88 * 1024)
    ...
    with args.out.open("w", encoding="utf-8") as handle:
        ...
        # The sketch must not hardcode a floor: it changes with the row.
        handle.write(f"#define SANOTTS_BENCH_ARENA_MIN {arena_min}\n")
        if is_nano:
    ...
    if is_nano:
        print(f"  arena  {arena_min:>7} B minimum ({arena_min/1024:.0f} KB) for {frames} frames")
